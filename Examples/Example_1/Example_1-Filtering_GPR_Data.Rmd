...
A list of the all column names is shown below.
```{r, echo=FALSE}
datacolumns <- read.csv("Examples/Example_1_Data/entireRunColumns.csv")
unique(names(datacolumns))[1:24]
```

The variables are defined as follows:

####These files were exported from Radan
...
* Scan.1: This was a repeated imported variable of scan and is not used.
...
####These files were added in the Excel file in order to aggregate the data efficiently
...
We then use a function called **mapply()** that takes a function and a set of inputs for that function.  If the set of inputs is a vector, the function will be applied over each set of inputs.  Three new columns *PartOfColumnBending*,*NSPartofColumnPunching*, and *EWPartofColumnPunching* are defined with the **mapply()** function in order to state the column (or NA) for a given rebar in the respective zone.
...
I hope you have seen the power and speed provided by R in regards to filtering and manipulating data.  R has even more capabilities shown beyond this basic functionality, as it is routinely used in data mining.
