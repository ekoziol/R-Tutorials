...
###Walkthrough
In order to get started with this analysis we will first load up a few packages.  We load ***ggplot2*** as our graphing library, ***reshape2*** to manipulate our data into a long Data Frame (to be explained later), and the ***plyr*** package for methods that allow ease of altering values.
...
The third step is to look only at the bridge haunch repairs that are in the current round of repairs.  We do this by filtering the data to select only those values within the repair set with **haunches[haunches$inRepairSet == 1,]**.
```{r, eval=FALSE}
library(ggplot2)
library(reshape2)
library(plyr)
